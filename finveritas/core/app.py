...
from src.data_verifier import run_verification, CredibilityReport, STATUS_PASS, STATUS_WARN, STATUS_FAIL, STATUS_SKIP, CredibilityCheck
...
def _render_shelf_company_panel(check: "CredibilityCheck") -> None:
    """Render the shelf/shell company filter as a dedicated always-visible section."""
    if check.status == STATUS_FAIL:
        border_colour = "#C94A3A"
        bg_colour     = "#1A0000"
        icon          = "🚫"
        badge_text    = "HIGH RISK"
        badge_bg      = "#C94A3A18"
        footer = (
            "Multiple signals indicate this entity may be a dormant or shell company. "
            "Nominee ownership, round-tripping, and layering are common risk vectors. "
            "Enhanced due diligence required before any credit or investment decision."
        )
    elif check.status == STATUS_WARN:
        border_colour = "#FFB000"
        bg_colour     = "#1A1000"
        icon          = "⚠️"
        badge_text    = "CAUTION"
        badge_bg      = "#FFB00018"
        footer = (
            "At least one shelf/shell company indicator detected. "
            "Verify operational activity, beneficial ownership, and business purpose "
            "before proceeding with any financial determination."
        )
    else:
        border_colour = "#3AB87A"
        bg_colour     = "#001A0D"
        icon          = "✅"
        badge_text    = "CLEAR"
        badge_bg      = "#3AB87A18"
        footer = "No dormant or shell company signals detected in the available financial data."

    render_section_header(
        "Shelf / Shell Company Filter",
        subtitle="Dormant entity detection · revenue-to-asset ratio · static financials · liability profile",
    )
    st.markdown(
        f'<div style="background:{bg_colour};border:1px solid {border_colour}33;'
        f'border-left:4px solid {border_colour};border-radius:4px;padding:14px 18px;margin:4px 0 8px 0;">'
        f'<div style="display:flex;align-items:center;gap:14px;">'
        f'<span style="font-size:22px;line-height:1;">{icon}</span>'
        f'<div>'
        f'<span style="font-size:9px;font-weight:700;color:{border_colour};background:{badge_bg};'
        f'padding:2px 7px;border-radius:2px;letter-spacing:0.15em;">{badge_text}</span>'
        f'<div style="font-size:11px;color:#CCCCCC;margin-top:5px;line-height:1.6;">'
        f'{html.escape(check.detail)}</div>'
        f'<div style="font-size:10px;color:#666;margin-top:5px;line-height:1.5;">{footer}</div>'
        f'</div></div></div>',
        unsafe_allow_html=True,
    )
